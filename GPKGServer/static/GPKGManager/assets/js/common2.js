...
function readGeoPackage(callback) {
  ...
  $.get('/static/GPKGManager/assets/templs/featureTable.tmpl', function(template) {
    featureTableTemplate = template;
  });
  Mustache.parse(featureTableTemplate);

  var tileTableTemplate = $('#tile-table-template').html();
  $.get('/static/GPKGManager/assets/templs/tileTable.tmpl', function(template) {
    tileTableTemplate = template;
  });
